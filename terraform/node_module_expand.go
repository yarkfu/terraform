...
func (n *evalValidateModule) Eval(ctx EvalContext) (interface{}, error) {
	...
	// Module all evaluate to single instances during validation, only to
	// create a proper context within which to evaluate. All parent modules will
	// be a single instance, but still get our address in the expected manner
	// anyway to ensure they've been registered correctly.
	for _, module := range expander.ExpandModule(n.Addr.Parent()) {
		// now set our own mode to single
		ctx.InstanceExpander().SetModuleSingle(module.Parent(), call)
	}
	return nil, nil
}
